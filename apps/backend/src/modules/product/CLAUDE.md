...
- **Publish guard** (`setStatus` → PUBLISHED): requires ≥1 active SKU, checked via raw SQL on `skus` (`hasActiveSku`, try/catch → false if table absent) — kept as raw SQL to avoid a service-level cycle.
- `Product.skus` is a `@OneToMany(() => Sku)` relation (inverse of `Sku.product`). Entity-level circular import is safe via the lazy `() => Sku` arrow; **use relative import paths** (`../../sku/entities/sku.entity`), not bare `src/...` — the typeorm CLI's ts-node can't resolve `src/` aliases and migration:run fails.
